  ...
  const startSession = async (question: string, key: string) => {
    ...
    try {
      ...
      const json = (await res.json()) as {
        session?: { id: string };
        github?: { ok?: boolean; skipped?: string; error?: string };
        error?: string;
      };
      if (!res.ok || !json.session) {
        throw new Error(json.error ?? "Failed to create session");
      }
      if (json.github?.skipped) {
        setError(
          `Session created locally. ${json.github.skipped} — sign in and pick a repo on the home page to sync to GitHub.`,
        );
      } else if (json.github?.error) {
        setError(`Session created, but GitHub sync failed: ${json.github.error}`);
      }
        ...
        <form onSubmit={handleCustomSubmit} className="space-y-3 p-4">
          ...
          <div className="flex items-center justify-between gap-3">
            <p className="text-xs text-zinc-500">
              Requirements are generated by OpenAI. First load may take a few seconds.
            </p>
            <button
              type="submit"
              disabled={pendingId !== null}
